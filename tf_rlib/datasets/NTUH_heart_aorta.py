...
class NTUH_HeartArota(datasets.Dataset):
    ...
    '''
    def __init__(self,
                 path='/mount/src/home/ntun/Whole_heart/NTUHv2/clara/data/',
                 max_crop_size=[144, 144, 144]):
        self.path = path
    ...
    def _load_data(self, path):
        ...
        for fname in img_fnames:
            ...
            seg = np.squeeze(
                nib.load(path + [f for f in seg_fnames
                                 if fname[:9] in f][0]).get_fdata())
    ...
    def _get_generator(self, x, y, val=False):
        def gen():
            ...
            for i in range(len(x) // FLAGS.bs):
                batch_x, batch_y = self._sample_data(x,
                                                     y,
                                                     idx[i * FLAGS.bs:(i + 1) *
                                                         FLAGS.bs],
                                                     val=val)
    ...
    def _sample_data(self, x, y, idx, val=False):
        ...
        for i in range(FLAGS.bs):
            if val:
                ...
                _x, _y = self._aug_transform([x[idx[i]], y[idx[i]]],
                                             min_crop_size=[48, 48, 48])
            xs += [_x]
            ys += [_y]
    ...
    def _aug_transform(self, data, min_crop_size):
        ...
        size = [
            np.random.randint(
                min_crop_size[i], x.shape[i] if
                x.shape[i] < self.max_crop_size[i] else self.max_crop_size[i])
    ...
    def _val_transform(self, data):
        ...
        size = x.shape[:3]
        x = x[size[0] // 2 - hd if size[0] // 2 - hd > 0 else 0:size[0] // 2 +
              hd, size[1] // 2 -
              hd if size[1] // 2 - hd > 0 else 0:size[1] // 2 + hd,
              size[2] // 2 - hd if size[2] // 2 - hd > 0 else 0:size[2] // 2 +
              hd]
        y = y[size[0] // 2 - hd if size[0] // 2 - hd > 0 else 0:size[0] // 2 +
              hd, size[1] // 2 -
              hd if size[1] // 2 - hd > 0 else 0:size[1] // 2 + hd,
              size[2] // 2 - hd if size[2] // 2 - hd > 0 else 0:size[2] // 2 +
              hd]
    ...
    def _pad_div_by_n(self, x, n=16):
        ...
        paddings = np.array([[(n - o) // 2, (n - o) - (n - o) // 2]
                             for o, n in zip(ori_size, new_size)] + [[0, 0]])
