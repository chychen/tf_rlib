...
class NVBump(datasets.Dataset):
    ...
    def _get_dsets(self):
        # load data
        X_defect = np.load('/mount/data/SPIL/Data/preprocessed/X_defect.npy')[:,172:428,172:428]
        X_pass = np.load('/mount/data/SPIL/Data/preprocessed/X_pass.npy')[:,172:428,172:428]
        X_defect_valid = np.load('/mount/data/SPIL/Data/preprocessed/X_defect_val.npy')
        X_pass_valid = np.load('/mount/data/SPIL/Data/preprocessed/X_pass_val.npy')
        ...
        valid_dataset = tf.data.Dataset.from_generator(gen_valid,
                                                       (tf.float32, tf.float32),
                                                       (tf.TensorShape([None, 224, 224, 3]), tf.TensorShape([None])))
        train_dataset = train_dataset.cache().shuffle(
            2000).prefetch(buffer_size=tf.data.experimental.AUTOTUNE)
